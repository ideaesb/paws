 ...
 <div id="header">
  ...
  <div id="headTitle">
   ...
   <div style="position:absolute;top:15px;left:660px;font-size:1.4em;font-weight: bold;">
    <a href="about.html" style="text-decoration: none;color:red;">Projects &amp; Activities Database</a>
  ...
  <table id="footerSiteMap">
     ...
     <tr>
      <th class="boxHeader" colspan="5">
       PaCIS 2010 Project and Activity Worksheet (Version 12.13.2010) 
...
<%
   ...
   {
%>
     <%= addSpaceTags(30) %> A D D <%= addSpaceTags(5) %> N E W <%= addSpaceTags(5) %> P R O J E C T  
<%
   }
   else
   {
...
%>        
     ...
     <tr>
       <td>
        ...
        <form name="project" action="projects" onsubmit="return checkform()" method="post">
        ...
        <input type="checkbox" name="training"  tabindex="1"    value="y"  <%= training %> />  Training and Capacity Building  <%= addSpaceTags(50) %>
        <input type="checkbox" name="education" tabindex="1"    value="y" <%= education %> />  Education  <%= addSpaceTags(50) %>
        <input type="checkbox" name="outreach"  tabindex="1"    value="y" <%= outreach %> />  Outreach <br /><br />

        <input type="checkbox" name="observing"     tabindex="1" value="y" <%= observing %> />  Observing Systems and Data Stewardship  <%= addSpaceTags(65) %>
        <input type="checkbox" name="dataservices"  tabindex="1" value="y" <%= dataservices %> />  Data Services  <br /><br />

        <input type="checkbox" name="variability"   tabindex="1" onclick="checkVariability()" value="y" <%= variability %> />Understanding Climate Variability and Change, including &quot;Best Practices&quot;    <br />
          <%= addSpaceTags(15) %><input type="checkbox" name="operational"  tabindex="1" onclick="alsoCheckVariability()" value="y"  <%= operational %> />Operational Products and Services (including Decision Support Tools)<br/>
          <%= addSpaceTags(15) %><input type="checkbox" name="research"     tabindex="1" onclick="alsoCheckVariability()" value="y"  <%= research %> />Research/Development<br/><br />
          <%= addSpaceTags(15) %><input type="checkbox" name="vhistorical"  tabindex="1" onclick="alsoCheckVariability()" value="y" <%= vhistorical %> />Historical Observations (hindcasts/climatologies)<br/>
          <%= addSpaceTags(15) %><input type="checkbox" name="vprojections" tabindex="1" onclick="alsoCheckVariability()" value="y" <%= vprojections %> />Projections (modeling and downscaling)<br/><br />

          <%= addSpaceTags(15) %>Parameter: <br />
         <%= addSpaceTags(20) %>Atmospheric: <%= addSpaceTags(5) %><input type="checkbox"  name="asurface"    <%= asurface %>  tabindex="1" onclick="alsoCheckVariability()"  value="y" />Surface (e.g., temperature, precipitation, wind)
                                             <%= addSpaceTags(10) %><input type="checkbox" name="upperair"    <%= upperair %>  tabindex="1" onclick="alsoCheckVariability()"  value="y" />Upper-Air
                                             <%= addSpaceTags(17) %><input type="checkbox" name="composition" <%= composition %> tabindex="1" onclick="alsoCheckVariability()" value="y" />Composition<br />
          <%= addSpaceTags(20) %>Oceanic:    <%= addSpaceTags(13) %><input type="checkbox" name="osurface"   <%= osurface %>  tabindex="1" onclick="alsoCheckVariability()" value="y" />Surface (e.g., SST, SSH, salinity, ocean color)
                                             <%= addSpaceTags(11) %><input type="checkbox" name="subsurface" <%= subsurface %> tabindex="1" onclick="alsoCheckVariability()" value="y" />Sub-surface (e.g., temp, salinity, nutrients, carbon, phytoplankton)<br />
          <%= addSpaceTags(20) %>Terrestrial:<%= addSpaceTags(10) %><input type="checkbox" name="terrestrial" <%= terrestrial %> tabindex="1" onclick="alsoCheckVariability()" value="y" />Terrestrial (e.g. surface water, glaciers and ice caps, land cover, biomass)<br /><br />
          <%= addSpaceTags(15) %>Time Frame: <%= addSpaceTags(10) %><input type="checkbox" name="seasonal" <%= seasonal %> tabindex="1" onclick="alsoCheckVariability()" value="y" />seasonal (outlook)
                                             <%= addSpaceTags(5) %><input type="checkbox"  name="annual"   <%= annual %> tabindex="1" onclick="alsoCheckVariability()" value="y" />intra-annual to decadal 
                                             <%= addSpaceTags(5) %><input type="checkbox"  name="decadal"  <%= decadal %> tabindex="1" onclick="alsoCheckVariability()" value="y" />muti-decadal (scenarios)<br /><br />



        <input type="checkbox" name="effect"  <%= effect %> tabindex="1" onclick="checkEffect()" value="y" />Understanding Climate Impacts and Informing Adaptation, including DSS<br />
          <%= addSpaceTags(15) %><input type="checkbox" name="impacts"  <%= impacts %>   tabindex="1" onclick="alsoCheckEffect()" value="y" />Climate Impacts               <br/>
            <%= addSpaceTags(50) %><input type="checkbox" name="ehistorical"  <%= ehistorical %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Historical Observations       
            <%= addSpaceTags(50) %><input type="checkbox" name="eprojections" <%= eprojections %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Projections<br /><br />


            <%= addSpaceTags(15) %><input type="checkbox" name="adaptation" <%= adaptation %>  tabindex="1" onclick="alsoCheckEffect()" value="y" />Climate Adaptation<br /><br />

            <%= addSpaceTags(15) %>Sector: 
              <%= addSpaceTags(20) %><input type="checkbox" name="phs"  <%= phs %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Public Health and Safety;
              <%= addSpaceTags(50) %><input type="checkbox" name="fwr"  <%= fwr %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Fresh Water Resources;
              <%= addSpaceTags(50) %><input type="checkbox" name="ene"  <%= ene %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Energy;<br/>
              
              <%= addSpaceTags(49) %><input type="checkbox" name="tcc"  <%= tcc %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Transportation/Communication, and Commerce;
              <%= addSpaceTags(6)  %><input type="checkbox" name="cpd"  <%= cpd %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Community Planning &amp; Development;<br />
              
              <%= addSpaceTags(49) %><input type="checkbox" name="scr"  <%= scr %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Social and Cultural Resources;
              <%= addSpaceTags(38) %><input type="checkbox" name="afi"  <%= afi %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Agriculture and Fisheries;
              <%= addSpaceTags(48) %><input type="checkbox" name="rat"  <%= rat %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Recreation and Tourism;<br />
              
              <%= addSpaceTags(49) %><input type="checkbox" name="eco"  <%= eco %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Ecosystems;
              <%= addSpaceTags(71) %><input type="checkbox" name="oth"  <%= oth %> tabindex="1" onclick="alsoCheckEffect()" value="y" />Other;<br /><br />
              
<!-- removed on December 10 2010

        <input type="checkbox" name="guidance"  <%= guidance %>   tabindex="1" onclick="checkGuidance()" value="0" />Guidance, including &quot;Best Practices&quot;  <br />
          <%= addSpaceTags(15) %><input type="checkbox" name="gvariability"   <%= gvariability %>  tabindex="1" onclick="alsoCheckGuidance()" value="0" />Climate Variability and Change
          <%= addSpaceTags(20) %><input type="checkbox" name="geffects"  <%= geffects %>  tabindex="1" onclick="alsoCheckGuidance()" value="0" />Climate Impacts and Informing Adaptation<br /><br />


        <input type="checkbox" name="apps" <%= apps %> tabindex="1" onclick="checkApps()" value="0" />Applications, including Decision-support Tools  <br />
          <%= addSpaceTags(15) %><input type="checkbox" name="avariability" <%= avariability %>  tabindex="1" onclick="alsoCheckApps()" value="0" />Climate Variability and Change
          <%= addSpaceTags(20) %><input type="checkbox" name="aeffects" <%= aeffects %> tabindex="1" onclick="alsoCheckApps()" value="0" />Climate Impacts and Informing Adaptation<br /><br />

-->

        <input type="checkbox" name="policies"  <%= policies %>  value="y" />Policies and Legislation   <%= addSpaceTags(47) %>
        <input type="checkbox" name="assessment"  <%= assessment %>  value="y" />Assessment and Evaluation<br /><br />
        ...
        <input type="checkbox" name="central"  <%= central %> tabindex="1" onclick="checkCNP()" value="y" />  Central North Pacific:&nbsp;
         ...
         <input type="checkbox" name="kauai" <%= kauai %> tabindex="1" onclick="alsoCheckHawaii()" value="y" />Kauai,<%= addSpaceTags(55) %>
         <input type="checkbox" name="hother" <%= hother %> tabindex="1" onclick="alsoCheckCNP()" value="y" />Other<br />
         ...
         <input type="checkbox" name="palau" <%= palau %> tabindex="1" onclick="alsoCheckWNP()" value="y" />Palau<%= addSpaceTags(101) %>
         <input type="checkbox" name="wother" <%= wother %> tabindex="1" onclick="alsoCheckWNP()" value="y" />Other<br />
        ...
        <input type="checkbox" name="fp"      <%= fp %>  tabindex="1" onclick="alsoCheckSP()" value="y" />French Polynesia;<%= addSpaceTags(10) %>
        <input type="checkbox" name="sother"  <%= sother %> tabindex="1" onclick="alsoCheckSP()" value="y" />Other<br />
        <input type="checkbox" name="otherregions"  <%= otherregions %>  tabindex="1" value="y" />  Other<br /><br /><br /> 

        <h2>Status</h2><%= addSpaceTags(25) %>           
        <input type="radio" name="status"  tabindex="1" value="<%= pb.COMPLETED %>"  <%= completed %> />Completed  <%= addSpaceTags(20) %>
        <input type="radio" name="status"  tabindex="1" value="<%= pb.ONGOING %>"  <%= ongoing %> />Ongoing  <%= addSpaceTags(20) %>
        <input type="radio" name="status"  tabindex="1" value="<%= pb.PLANNED %>"  <%= planned %> />Planned  <%= addSpaceTags(20) %>
        <input type="radio" name="status"  tabindex="1" value="<%= pb.PROPOSED %>"  <%= proposed %> />Proposed <%= addSpaceTags(20) %>
        <input type="radio" name="status"  tabindex="1" value="<%= pb.UNKNOWN %>"  <%= unknown %> />Not Available <br /><br /><br />
