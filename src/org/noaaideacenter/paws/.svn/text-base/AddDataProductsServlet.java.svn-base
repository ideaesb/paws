...
public class AddDataProductsServlet extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		//
   		...
   		if (ParameterDAO.parse(searchbyx))
   		{
   		  // regular listing
   		  request.setAttribute("searchbyx", "true");
   		  request.getRequestDispatcher("dataproducts3.jsp").include(request, response);
   		}
   		else
   		{
   			// admin listing
    		request.getRequestDispatcher("/dnps.jsp").include(request, response);
   		}
	...
	private void setProjectBean(HttpServletRequest request)
	{
		...
		pb.setEcvAtmospheric(request.getParameter("ecvatmospheric"));
		pb.setEcvOceanic(request.getParameter("ecvoceanic"));
		pb.setEcvTerrestrial(request.getParameter("ecvterrestrial"));
		
		pb.setPhyInsitu(request.getParameter("phyinsitu"));
		pb.setPhySatellite(request.getParameter("physatellite"));
		pb.setPhyModel(request.getParameter("phymodel"));
		pb.setPhyReanalysis(request.getParameter("phyreanalysis"));
		pb.setPhyBathymetry(request.getParameter("phybathymetry"));
		pb.setPhyImagery(request.getParameter("phyimagery"));
		
		pb.setProdHindcasts(request.getParameter("prodhindcasts"));
		pb.setProdOutlooks(request.getParameter("prodoutlooks"));
		pb.setProdProjections(request.getParameter("prodprojections"));
		pb.setProdGuidance(request.getParameter("prodguidance"));
		pb.setProdApplications(request.getParameter("prodapplications"));
		
		
	    pb.setNonPhysicalEnvironmental(request.getParameter("enviro"));
	    pb.setNonPhysicalSocioCultural(request.getParameter("socio"));
	    pb.setNonPhysicalEconomic(request.getParameter("econo"));
		pb.setNonPhysical(request.getParameter("nonphysical"));
        ...
        pb.setUrl(request.getParameter("url"));
	...
	{
		AddDataProductsServlet a = new AddDataProductsServlet(); 
		StringBuffer sb = new StringBuffer("Message");
		System.out.println(a.getFooter());
	}
